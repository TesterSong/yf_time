...
class yf_time_struct(object):
    ...
    def show(self, view=False, rt_str=True):
        ...
        if rt_str:
            ...
            tmp=str(int(self.year) + 2000) + "-" + str(int(self.month)) + "-" + str(int(self.day)) + " " + str(
                int(self.hour)) + ":" + str(int(self.minute)) + ":" + str(int(self.sec))
            dd = datetime.datetime.strptime(tmp,"%Y-%m-%d %H:%M:%S")
...
if __name__ == '__main__':
    t = utc_time()
    print(t.seconds_to_utc(1248415200).show(False, True))
    #time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(157845151+1388505600))
    #print(time_str)
